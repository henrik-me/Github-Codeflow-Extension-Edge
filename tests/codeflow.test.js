import { assert, test, createPage, run } from "./harness.js";

const prUrl = location.origin + "/owner/repo/pull/10";
...
test("React title icon is outside the screen-reader-only PR number", async t => {
    const page = await createPage(t, reactHeader);
    await page.settle();
...
test("legacy Files changed heading receives an icon", async t => {
    const page = await createPage(t, legacyHeader, prUrl + "/files#diff-123");
    await page.settle();
    assertLink(page.document.querySelector("h1"));
});
...
test("repeated navigation events do not duplicate header icons", async t => {
    const page = await createPage(t, legacyHeader);
    await page.settle();
...
test("React list uses PR links rather than generated row IDs", async t => {
    const page = await createPage(t, reactList, location.origin + "/owner/repo/pulls?q=is%3Aopen");
    await page.settle();
...
test("legacy list uses the linked PR number, not the internal issue ID", async t => {
    const page = await createPage(t, `
        ...
        </div>`, location.origin + "/owner/repo/pulls/");
    await page.settle();
...
test("PR list ignores external, invalid, and non-PR title links", async t => {
    const page = await createPage(t, `
        ...
        </ul>`, location.origin + "/owner/repo/pulls");
    await page.settle();
...
for (const suffix of ["/", "/files?diff=split#diff-123", "/commits/abc123",
    ...
    test("PR destination is canonical on " + suffix, async t => {
        const page = await createPage(t, reactHeader, prUrl + suffix);
        await page.settle();
        assertLink(page.document.querySelector("h1"));
    });
}
...
test("URL normalization preserves a repository named commits", async t => {
    const url = location.origin + "/owner/commits/pull/10";
    const page = await createPage(t, reactHeader, url + "/commits");
    await page.settle();
...
test("a list row reused by filtering gets the new destination without duplicates", async t => {
    const page = await createPage(t, reactList, location.origin + "/owner/repo/pulls");
    await page.settle();
...
test("repository-to-list-to-PR navigation works without reloading the content script", async t => {
    const page = await createPage(t, "<h1>Repository</h1>", location.origin + "/owner/repo");
    await page.settle();
    assert.equal(page.document.querySelectorAll(linkSelector).length, 0);
...
test("list pages do not decorate unrelated page headings", async t => {
    const page = await createPage(t, reactHeader + reactList, location.origin + "/owner/repo/pulls");
    await page.settle();
...
run().catch(error => {
    ...
    document.querySelector("#summary").textContent = error.stack || String(error);
    console.error(error);
});
